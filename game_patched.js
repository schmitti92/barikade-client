...
(() => {
  ...
  function applyRemoteState(remote){
    ...
    // Keep phase + placement highlights in sync across all clients
    if(typeof st._phase === "string"){
      phase = st._phase;
    }else{
      // Best-effort fallback (older snapshots)
      if(state.winner) phase = "game_over";
      else if(state.dice==null) phase = "need_roll";
      else phase = "need_move";
    }
  ...
  function serializeState(){
    ...
    // Online meta (clients need these to keep UI + phase in sync)
    st.players = [...PLAYERS];
    st.playerOrder = Array.isArray(state.playerOrder) ? state.playerOrder : (st.playerOrder||[]);
    st.started = !!state.started;
    st._phase = phase;
  ...
  function updateTurnUI(){
    ...
    // Online: lock UI if it's not your turn
    if(netMode!=="offline" && myColor && !isMyTurn){
      rollBtn.disabled = true;
      endBtn.disabled = true;
      if(skipBtn) skipBtn.disabled = true;
    }
...
function anyPiecesAtNode(nodeId){
  ...
  function onPointerDown(ev){
    const online = (netMode!=='offline');
    if(online && state && !state.started){ return; }
    if(online && myColor && state && state.currentPlayer !== myColor){ return; }
