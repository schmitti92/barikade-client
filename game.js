...
(() => {
  ...
  const myColorLabel = $("myColorLabel");
  ...
  let phase="need_roll"; // need_roll | need_move | placing_barricade | waiting_start | game_over
  ...
  // Color assignment (online)
  let myColor = null;           // e.g. 'red'
  let myAssignedIndex = -1;     // position in playerOrder

  function connectedCount(){
    return (lastNetPlayers||[]).filter(p=>p && p.connected!==false).length;
  }

  function updateMyColorLabel(){
    if(!myColorLabel) return;
    if(!myColor){ myColorLabel.textContent = "–"; return; }
    myColorLabel.textContent = PLAYER_NAME[myColor] || myColor;
    myColorLabel.style.color = COLORS[myColor] || "var(--muted)";
  }

  function computeMyColorFromStateOrRoster(){
    // Prefer fixed playerOrder from authoritative state (stable colors)
    if(state && Array.isArray(state.playerOrder) && state.playerOrder.length){
      const idx = state.playerOrder.indexOf(clientId);
      myAssignedIndex = idx;
      if(idx >= 0){
        const colors = (state.players && Array.isArray(state.players)) ? state.players : PLAYERS;
        myColor = colors[idx] || null;
        updateMyColorLabel();
        return;
      }
    }

    // Fallback (before game started): derive from roster order
    const roster = (lastNetPlayers||[]).filter(p=>p && p.connected!==false);
    const idx = roster.findIndex(p => (p.id||"") === clientId);
    myAssignedIndex = idx;
    myColor = (idx>=0) ? DEFAULT_PLAYERS[idx] : null;
    updateMyColorLabel();
  }
  ...
  function setNetPlayers(list){
    ...
    computeMyColorFromStateOrRoster();
    updateMyColorLabel();

    if(startBtn){
      // Online-only control
      startBtn.style.display = (netMode==="offline") ? "none" : "inline-flex";
      const canStart = (netMode==="host" && connectedCount()>=2);
  ...
  function applyRemoteState(remote){
    ...
    // Sync host UI-state so clients can actually roll/move at the right time
    if(typeof st._phase === "string") phase = st._phase;
    if(Array.isArray(st._placingChoices)) placingChoices = st._placingChoices;
    if(st._selected) selected = st._selected;

    // Cleanup (optional)
    // delete state._phase; delete state._placingChoices; delete state._selected;
  ...
  function broadcastState(kind="state"){
    if(netMode!=="host") return;
    // Host is authoritative: also sync local UI-only variables (phase/placing/selection)
    const st = serializeState();
    st._phase = phase;
    st._placingChoices = Array.isArray(placingChoices) ? placingChoices : [];
    st._selected = selected || null;

    const payload = {type: kind, room: roomCode, state: st, ts:Date.now()};
    wsSend(payload);
  ...
  function handleRemoteIntent(intent){
    const t = intent.type;
    if(t==="roll"){ rollDice(); broadcastState("state"); return; }
    if(t==="end"){ if(phase!=="placing_barricade" && phase!=="game_over") nextPlayer(); broadcastState("state"); return; }
    if(t==="skip"){ if(phase!=="placing_barricade" && phase!=="game_over"){ toast("Runde ausgesetzt"); nextPlayer(); } broadcastState("state"); return; }
    if(t==="reset"){ newGame(); broadcastState("snapshot"); return; }
    if(t==="select"){ selected = intent.selected || null; draw(); return; }
    if(t==="move"){
  ...
  function startOnlineGame(){
    ...
    const roster = (lastNetPlayers||[]).filter(p=>p && p.connected!==false);
    if(roster.length < 2){ toast("Mindestens 2 Spieler nötig"); return; }

    // Fix players + order (stable colors, even if reconnect/disconnect later)
    const count = Math.max(2, Math.min(4, roster.length));
    setPlayers(count);

    // Authoritative assignment: roster order => colors
    const order = roster.slice(0, count).map(p => p.id);
  ...
  function updateTurnUI(){
    ...
    const online = (netMode !== 'offline');
    const started = !(online) || !!state.started;
    const isMyTurn = (!myColor) ? (netMode==='host') : (myColor === c);

    if(!started && online){
      turnText.textContent = 'Warten auf Start (Host)…';
      turnDot.style.background = 'rgba(255,255,255,0.3)';
    ...
    rollBtn.disabled = (phase!=="need_roll");
    endBtn.disabled = (phase==="need_roll"||phase==="placing_barricade"||phase==="game_over");
    if(skipBtn) skipBtn.disabled = (phase==="placing_barricade"||phase==="game_over");

    // Online: lock UI if it's not your turn (or if you are not assigned yet)
    if(netMode!=="offline"){
      if(netMode==="client" && !myColor){
        // Not assigned yet -> treat as spectator until roster/state arrives
        rollBtn.disabled = true;
        endBtn.disabled = true;
        if(skipBtn) skipBtn.disabled = true;
...
function anyPiecesAtNode(nodeId){
  ...
  function onPointerDown(ev){
    ...
    if(online){
      ...
      if(netMode==="client"){
        if(!myColor) return;
        if(state && state.currentPlayer !== myColor) return;
      }
    }
    if; }
    ...
    if(phase==="need_move"){
      ...
      if(trySelectAtNode(hit)) { if(netMode==="client"){ sendIntent({type:"select", selected}); } draw(); return; }
  ...
  rollBtn.addEventListener("click", () => {
    const online = (netMode!=="offline");
    if(online && state && !state.started){ toast('Warten auf Start'); return; }
    if(online && netMode==="client" && !myColor){ toast('Du bist (noch) nicht zugeordnet'); return; }
    if(online && netMode==="client" && !myColor){ toast('Du bist (noch) nicht zugeordnet'); return; }
    if(online && netMode==="client" && !myColor){ toast('Du bist (noch) nicht zugeordnet'); return; }
    if(online && myColor && state && state.currentPlayer!==myColor){ toast('Du bist nicht am Zug'); return; }
  ...
  resetBtn.addEventListener("click", () => {
    if(netMode==="client"){ toast('Nur Host kann Reset'); return; }
