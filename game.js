...
let goalId=null;
let houseSlotsByColor={red:[],blue:[],green:[],yellow:[]};
...
function buildGraph(){
  ...
  goalId=null;
  houseSlotsByColor={red:[],blue:[],green:[],yellow:[]};
  ...
  for(const n of board.nodes){
    ...
    // houses (optional – for drawing pieces still "im Haus")
    if(n.kind==="house" && n.flags?.houseColor && Number.isFinite(n.flags.houseSlot)){
      const c = n.flags.houseColor;
      const slot = Math.max(1, Math.min(5, Number(n.flags.houseSlot))) - 1;
      if(houseSlotsByColor[c]){
        houseSlotsByColor[c][slot] = n; // keep exact slot position from board.json
      }
    }
  }
...
function fitToBoard(){
  const nodes = board.nodes.filter(n=>n.kind==="board" || n.kind==="house");
...
let state=null;
let lastStateForAnim=null; // for client-side move animation on snapshots
// state = {
...
function applyState(remote){
  ...
  // detect single-piece movement for smooth animation (clients)
  const prev = lastStateForAnim;
  let animCandidate = null; // {color,index, from, to}

  if(prev && prev.pieces && st.pieces){
    try{
      const colors = new Set([...(prev.players||[]), ...(st.players||[])]);
      for(const c of colors){
        const a = prev.pieces[c]||[];
        const b = st.pieces[c]||[];
        for(let i=0;i<Math.max(a.length,b.length,5);i++){
          const from = a[i]?.pos;
          const to = b[i]?.pos;
          if(from!==to && typeof from==="string" && typeof to==="string"){
            // ignore big jumps that are likely resets (e.g., start/reset/sync)
            // but allow house <-> start and normal board moves
            if(!animCandidate) animCandidate = {color:c, index:i, from, to};
            else { animCandidate = null; break; } // more than one change -> no animation
          }
        }
        if(animCandidate===null) break;
      }
    }catch{ animCandidate = null; }
  }
  ...
  // start animation if we detected exactly one moved piece
  if(animCandidate && state.gamePhase==="running"){
    const {color,index,from,to} = animCandidate;

    const resolveNode = (pos, idx) => {
      if(pos==="house") return houseSlotsByColor?.[color]?.[idx] || (startByColor[color] ? nodeById.get(startByColor[color]) : null);
      if(pos==="goal") return goalId ? nodeById.get(goalId) : null;
      return nodeById.get(pos);
    };

    const A = resolveNode(from, index);
    const B = resolveNode(to, index);

    if(A && B){
      startAnimPoints(color, index, [{x:A.x, y:A.y}, {x:B.x, y:B.y}]);
    }
  }

  // store snapshot for next diff (keep barricades as array to avoid Set cloning issues)
  lastStateForAnim = {
...
function startAnimPoints(color,index, pts){
  if(!Array.isArray(pts) || pts.length===0){ anim=null; return; }
  startAnimPoints(color,index, pts);
}
 // {color,index, points:[{x,y}], t, dur}
  ...
  if(state){
    ...
    for(const c of state.players){
      for(let i=0;i<5;i++){
        ...
        // draw "house" pieces on house slots (from board.json) so you see all 5 at start
        let node = null;
        if(pos==="house"){
          node = houseSlotsByColor?.[c]?.[i] || (startByColor[c] ? nodeById.get(startByColor[c]) : null);
        }else if(pos==="goal"){
          node = goalId ? nodeById.get(goalId) : null;
        }else{
          node = nodeById.get(pos);
        }
        if(!node) continue;

        let P=worldToScreen(node);

        // override with anim position (only for on-board moves)
        if(pos!=="house" && pos!=="goal"){
          // handled below (kept for compatibility)
        }
        ...
        const off = (i-2)*4;
        ctx.beginPath();
        ctx.arc(P.x + off, P.y - off, nodeRadius, 0, Math.PI*2);
        ...
        if(selected && selected.color===c && selected.index===i){
          ctx.beginPath();
          ctx.arc(P.x + off, P.y - off, nodeRadius+4, 0, Math.PI*2);
