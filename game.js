(() => {
  ...
  function setNetPlayers(list){
    ...
    if(!ui.netPlayers) return;

    if(!lastNetPlayers.length){
      ui.netPlayers.textContent = "–";
      return;
    }

    const lines = lastNetPlayers.map(p=>{
      const name = p.name || "Spieler";
      const role = p.isHost ? "(host)" : "(client)";
      const col  = p.color ? `· ${PLAYER_NAME[p.color] || p.color}` : "";
      const con  = (p.connected === false) ? " ✖" : " ✔";
      return `${name} ${role} ${col}${con}`.trim();
    });

    ui.netPlayers.textContent = lines.join(String.fromCharCode(10));
  }
