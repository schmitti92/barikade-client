...
(() => {
  ...
  function setPlayers(arg){
    // arg can be a number (2-4) or an array of colors like ["red","blue",...]
...
let placingChoices = [];
  ...
  function setNetPlayers(list){
    ...
    for(const p of lastNetPlayers){
      if(p && p.id) rosterById.set(p.id, p);
    }

    // determine my assigned color (if server sent it)
    ...
    // Host: keep active player list in sync with connected clients
  ...
  function applyRemoteState(remote){
    ...
    // Sync phase + placing choices (authoritative from host)
    if(typeof st.phase === "string"){
      phase = st.phase;
    }else{
      // Backwards compatibility: derive a reasonable phase
      if(st.winner) phase = "game_over";
      else if(st.dice==null) phase = "need_roll";
      else phase = "need_move";
    }
    placingChoices = Array.isArray(st.placingChoices) ? st.placingChoices : [];

    // Recompute local legal moves for UI when host rolled
  ...
  function serializeState(){
    ...
    st.players = (state && state.players) ? [...state.players] : [...PLAYERS];
    st.phase = phase;
  ...
  function handleRemoteIntent(intent, senderId=""){
    ...
    // Only the active player's client can trigger turn actions
    const mustBeTurnPlayer = () => {
      if(!senderColor) return false;
      return senderColor === state.currentPlayer;
    };
    ...
    if(t==="skip"){
      if(!mustBeTurnPlayer()) return;
      if(phase!=="placing_barricade" && phase!=="game_over"){
        toast("Runde ausgesetzt"); nextPlayer();
      }
      broadcastState("state"); return;
    }
    if(t==="reset"){
      // prevent grief: only host may reset
...
if(skipBtn) skipBtn.disabled = (phase==="placing_barricade"||phase==="game_over") || !isMyTurn;
  ...
  function nextPlayer(){
    const order = state.players && state.players.length ? state.players : PLAYERS;
...
function anyPiecesAtNode(nodeId){
  ...
  function onPointerDown(ev){
    ...
    if(netMode==="client" && !isMyTurn && (phase==="placing_barricade" || phase==="need_move" || phase==="need_roll")){
      toast("Du bist nicht dran");
      return;
    }
  ...
  rollBtn.addEventListener("click", () => {
    if(netMode==="client"){
      if(!myColor){ toast("Noch keine Farbe zugeteilt"); return; }
      if(myColor!==state.currentPlayer){ toast("Du bist nicht dran"); return; }
      sendIntent({type:"roll"});
      return;
    }
    rollDice();
    if(netMode==="host") broadcastState("state");
