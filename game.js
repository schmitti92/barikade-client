(() => {
  ...
  function setNetPlayers(list){
    ...
    if(netMode==="host" && state && state.gamePhase !== "running"){
    ...
    if(startRow && startBtn && state){
      const activeCount = getActiveColors().length;
      const canStart = (netMode==="host") && (state.gamePhase !== "running") && (activeCount >= 2);
      startRow.style.display = (netMode==="host") ? "flex" : "none";
...
function broadcastState(kind="state"){
  ...
  function updateTurnUI(){
    ...
    const isRunning = (state && state.gamePhase === "running");

    const isMyTurn = (netMode==="offline") ? true : (myColor && myColor===state.currentPlayer);
    rollBtn.disabled = !isRunning || (phase!=="need_roll") || !isMyTurn;
    endBtn.disabled  = !isRunning || (phase==="need_roll"||phase==="placing_barricade"||phase==="game_over") || !isMyTurn;
    if(skipBtn) skipBtn.disabled = !isRunning || (phase==="placing_barricade"||phase==="game_over") || !isMyTurn;
  ...
  if(startBtn) startBtn.addEventListener("click", () => {
    ...
    if(!state){ toast("State fehlt – bitte kurz warten"); return; }

    const active = getActiveColors();
    if(active.length < 2){ toast("Mindestens 2 Spieler/Farben nötig"); return; }

    // Spieler beim Start locken
    setPlayers(active);
    state.players = [...PLAYERS];
...
rollBtn.addEventListener("click", () => {
    if(netMode==="client"){
      ...
      if(!state || state.gamePhase !== "running"){ toast("Warte: Host muss starten"); return; }
