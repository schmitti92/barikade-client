(() => {
  ...
  let state=null;
  let anim=null; // piece animation (local visual)
  let lastMoveSig=null;
    ...
    if(netMode==="host" && state){
      ...
      if(!same){
        ...
        if(!state.players.includes(state.currentPlayer)){
          ...
          setPhase(state.started ? "need_roll" : "lobby");
          state.dice=null;
        }
...
if(netPlayersEl){
  if(!lastNetPlayers.length){ netPlayersEl.innerHTML = '<span class="muted">–</span>'; }
  else{
    const pills = lastNetPlayers.map(p=>{
      const c = p.color || "";
      const label = (p.role==="host" ? "Host" : "Client");
      const you = (p.id===clientId) ? " du" : "";
      const active = (c && c===state.currentPlayer) ? " active" : "";
      const dot = c ? `<span class="colorDot ${c}"></span>` : `<span class="colorDot none"></span>`;
      return `<span class="playerPill${active}">${dot}<span class="playerTxt">${label}${you}</span></span>`;
    }).join(" ");
    netPlayersEl.innerHTML = `<div class="playersPills">${pills}</div>`;
  }
}
  }
  ...
  function applyRemoteState(remote){
    ...
    state = st;
    state.colorByClient = state.colorByClient || {};
    if(typeof state.started !== "boolean") state.started = false;
    // play last move animation if a new move arrived
    if(state?.lastMove){
      const lm = state.lastMove;
      const sig = `${lm.ts}|${lm.piece?.color}|${lm.piece?.index}`;
      if(sig && sig !== lastMoveSig){
        lastMoveSig = sig;
        if(lm.piece && lm.path && lm.path.length>1){
          anim = { color: lm.piece.color, index: lm.piece.index, path: lm.path.slice(), started: performance.now(), stepDur: 180 };
        }
      }
    }
    ...
    if(diceEl) setDiceDisplay(state.dice==null?null:state.dice);
    updateTurnUI(); draw();
  }
  ...
  function updateTurnUI(){
    ...
    turnText.textContent = state.winner ? `${PLAYER_NAME[state.winner]} gewinnt!` : ((phase==="lobby" || state.started===false) ? "Lobby – Farbe wählen & Start drücken" : `${PLAYER_NAME[c]} ist dran`);
...
function setDiceDisplay(v){
  if(!diceEl) return;
  if(v==null){ diceEl.textContent = "–"; diceEl.dataset.val=""; return; }
  diceEl.textContent = DICE_FACE[v] || String(v);
  diceEl.dataset.val = String(v);
}

function animateDice(finalVal){
  // quick roll animation for nicer feel
  const start = performance.now();
  const dur = 420;
  const tick = () => {
    const t = performance.now() - start;
    if(t >= dur){ setDiceDisplay(finalVal); return; }
    const v = 1 + Math.floor(Math.random()*6);
    diceEl.textContent = DICE_FACE[v];
  ...
  function rollDice(){
    if(state && state.started===false) return;
...
// remember last move for nice animation on all clients
state.lastMove = { piece: move.piece, path: move.path || [], ts: Date.now() };
anim = { color, index, path: (move.path||[]), started: performance.now(), stepDur: 180 };
...
function getAnimScreenPos(){
  if(!anim || !anim.path || anim.path.length<2) return null;
  const elapsed = performance.now() - anim.started;
  const totalSteps = anim.path.length - 1;
  const stepDur = anim.stepDur || 180;
  const p = Math.max(0, elapsed / stepDur);
  const i = Math.min(totalSteps-1, Math.floor(p));
  const frac = Math.min(1, p - i);
  const a = nodeById.get(anim.path[i]);
  const b = nodeById.get(anim.path[i+1]);
  if(!a || !b) return null;
  const wx = a.x + (b.x-a.x)*frac;
  const wy = a.y + (b.y-a.y)*frac;
  const s = worldToScreen(wx, wy);
  // auto-finish
  if(elapsed >= totalSteps*stepDur){ anim = null; }
  return s;
}
  ...
  function draw(){
    ...
    for(const n of board.nodes){
      ...
      if(n.kind==="board"){
        ...
        else if(n.flags?.run) fill=COLORS.node; // run fields look neutral
  ...
  function handleRemoteIntent(intent, senderId=""){
    ...
    if(t==="chooseColor"){
      ...
      // reserve unique color
      state.colorByClient = state.colorByClient || {};
      // if color already taken by other connected client -> reject
      const takenBy = Object.entries(state.colorByClient).find(([cid, c])=>c===col && cid!==senderId);
      if(takenBy) return;

      state.colorByClient[senderId] = col;

      // also keep host's own selection if not set
      if(!state.colorByClient[clientId] && netMode==="host"){
        // no-op
      }

      // if game not started, update active players preview
    ...
    if(t==="startGame"){
      ...
      const chosen = Object.values(state.colorByClient||{});
...
// animated piece (visual only) so you see what the other player did
if(anim){
  const s = getAnimScreenPos();
  if(s){
    drawStack([{color: anim.color, index: anim.index}], s.x, s.y, r);
  }
}
)()
  function getColorForId(id){
    // Prefer host-authoritative color mapping stored in game state (works even if server doesn't support colors)
    const st = state;
    if(st && st.colorByClient && st.colorByClient[id]) return st.colorByClient[id];
    const p = rosterById.get(id);
    return (p && p.color) ? p.color : null;
  }

  if(startBtn) startBtn.addEventListener("click", () => {
    if(netMode==="client"){ toast("Nur Host kann starten"); return; }
    if(!state || state.started) return;
    handleRemoteIntent({type:"startGame"}, clientId);
  });
