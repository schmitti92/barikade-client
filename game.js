...
(() => {
  ...
  // legacy/global flag to avoid "isOnline is not defined" if old code references it
  ...
  function buildClientColorMap(playersList){
    const list = Array.isArray(playersList) ? playersList : (Array.isArray(lastNetPlayers) ? lastNetPlayers : []);
    const map = {};
    // roster() on server sorts host first. We map roster order => PLAYERS order.
    const n = Math.min(PLAYERS.length, list.length);
    for(let i=0;i<n;i++){
      const pid = list[i]?.id;
      if(pid) map[pid] = PLAYERS[i];
    }
    return map;
  }

  function computeMyColor(){
    if(netMode==="offline"){ myColor=null; return; }
    const map = (state && state.clientColors) ? state.clientColors : buildClientColorMap();
    myColor = map && clientId ? (map[clientId] || null) : null;
  }
  ...
  function isFreshGame(){
    if(!state || state.dice!=null) return false;
    for(const c of PLAYERS){
      const arr = state.pieces?.[c] || [];
      for(const p of arr){
        if(p.pos !== "house") return false;
      }
    }
    return true;
  }
  ...
  function setNetPlayers(list){
    ...
    // Auto-Playercount (Host): 2/3/4 Spieler je nach verbundenen Clients
    if(netMode==="host"){
      const connectedCount = Math.max(1, lastNetPlayers.length);
      const target = Math.max(2, Math.min(4, connectedCount));
      if(target !== PLAYERS.length && isFreshGame()){
        setPlayers(target);
        newGame();
        // keep host as current player after reset
  ...
  function connectWS(){
    ...
    ws.onmessage = (ev) => {
      ...
      if(type==="intent" || type==="move" || type==="cmd"){
        ...
        if(it) handleRemoteIntent(it, msg.clientId);
        return;
      }
  ...
  function serializeState(){
    ...
    // NEW: seat mapping (clientId -> Farbe) so jeder nur seine Farbe steuert
    st.clientColors = buildClientColorMap();
return st;
  }
  ...
  function handleRemoteIntent(intent, senderClientId){
    const t = intent.type;

    // Validate: only the active player's client may act, and only for its own color
    const map = (state && state.clientColors) ? state.clientColors : buildClientColorMap();
    const senderColor = map && senderClientId ? (map[senderClientId] || null) : null;

    const mustBeTurn = ["roll","end","skip","select","move","placeBarricade"];
    if(mustBeTurn.includes(t)){
      if(!senderColor) return;
      if(state.currentPlayer !== senderColor) return;
    }

    // Prevent controlling other colors
    if((t==="select" || t==="move") && intent.selected && intent.selected.color && senderColor){
      if(intent.selected.color !== senderColor) return;
    }
  ...
  function updateTurnUI(){
    ...
    const youTxt = (netMode==="offline") ? "" : (myColor ? ` · Du bist: ${PLAYER_NAME[myColor]}` : " · Du bist: –");
    turnText.textContent = state.winner
      ? `${PLAYER_NAME[state.winner]} gewinnt!`
      : `${PLAYER_NAME[c]} ist dran${youTxt}`;

    turnDot.style.background = COLORS[c];

    const myTurn = isMyTurn();

    rollBtn.disabled = (phase!=="need_roll") || (netMode!=="offline" && !myTurn);
    endBtn.disabled  = (phase==="need_roll"||phase==="placing_barricade"||phase==="game_over") || (netMode!=="offline" && !myTurn);
    if(skipBtn) skipBtn.disabled = (phase==="placing_barricade"||phase==="game_over") || (netMode!=="offline" && !myTurn);
  }
...
function anyPiecesAtNode(nodeId){
  ...
  function onPointerDown(ev){
    ...
    // Online: only the player whose turn it is may interact with pieces/barricades
    const interactivePhase = (phase==="placing_barricade" || phase==="need_move");
    if(netMode!=="offline" && interactivePhase && !isMyTurn()){
      // allow pan/zoom only
      isPanning=true;
  ...
  rollBtn.addEventListener("click", () => {
    if(netMode!=="offline" && !isMyTurn()){ toast("Du bist nicht dran"); return; }
  ...
  resetBtn.addEventListener("click", () => {
    if(netMode==="client"){ toast("Reset nur beim Host"); return; }

    if(netMode==="client"){ sendIntent({type:"reset"}); return; }
    newGame();
    if(netMode==="host") broadcastState("snapshot");
  });
